...
- **`SimulationInputs`** — reads a workbook's own account of what is uncertain.

  `ExcelRecognizer` reads *model* structure: where the period axis runs, which rows
  are series, how a formula lags its neighbours. It has no way to tell a hard-coded
  assumption from a random draw, because topologically they are identical — a cell
  fed by nothing, feeding something.
  ...
  carrying `PsiOutput()` is a reported result and says so. `SimulationInputs.survey`
  bridges `SwiftXLSX.WorkbookValueProvider` to SwiftExcelFunctions' `ModelSurveyor`
  and returns both, plus any property function left unmodelled.

  Recognition, not evaluation — no add-in, no seed, no simulation engine. That is
  the point rather than a limitation: an archived Risk Solver model has none of
  those, and its formulas still say what they are.
...
- **Depends on SwiftExcelFunctions**, pinned `.upToNextMinor(from: "0.7.1")`, and
  catches up two pins that had drifted a long way: BusinessMath 2.9.0 → 2.15.0 and
  SwiftXLSX 0.21.0 → 0.23.0. All three jumps were additive; no source here needed
  changing for any of them.

  This repo had deliberately trailed the family while the function library was being
  built. It no longer should: 269 Excel and Risk Solver functions now answer, and the
  translation layer is the thing that should be using them.

- **The intra-family pins are ranges rather than `exact:`**, and the reasoning is
  recorded in `CLAUDE.md`. Two `exact:` pins on one shared package deadlock the moment
  they differ, which is what stopped this repo resolving once before.

  **One pin still has not had that reasoning applied to it.** BusinessMath is still
  `exact: "2.15.0"` here, while SwiftExcelFunctions' `main` has moved to
  `.upToNextMinor(from: "3.0.0-alpha.3")`. Those are unsatisfiable together. This
  release resolves because it pins SwiftExcelFunctions **v0.7.1**, whose manifest still
  asks for `from: "2.11.0"` — so the break arrives at the *next* SwiftExcelFunctions
  bump, not at this one. Documented in `CLAUDE.md` under "The pin that is about to
  break", and it is the first item in the handoff. Shipping a known scheduled failure
  is deliberate: the alternative was holding this release behind an upstream tag that

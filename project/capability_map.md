...
**Last reviewed:** 2026-09-10 (reviewed for v0.8.0)
...
## Reading a Workbook's Own Uncertainty

**Key types:** `SimulationInputs`, `SimulationInputs.Survey`
**Interfaces:** `SimulationInputs.survey(_:)`
**Applications:** Recovering which cells a Risk Solver workbook treats as uncertain, and which it reports as results

Recognition reads *structure* and cannot tell a hard-coded assumption from a random draw — the two
are topologically identical, a cell fed by nothing and feeding something. A workbook built with
Risk Solver already answers the question in its formulas: `PsiTriangular(80, 100, 130)` is an
uncertain input and says so; `PsiOutput()` is a reported result and says so.

This bridges `SwiftXLSX.WorkbookValueProvider` to SwiftExcelFunctions' `ModelSurveyor` and returns
both, plus any property function left unmodelled — so what is *not* recovered is visible rather
than silently absent.

Added 0.8.0, with the SwiftExcelFunctions dependency that makes it possible.
